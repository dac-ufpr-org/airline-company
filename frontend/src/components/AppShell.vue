<template>
  <div class="flex min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 overflow-hidden">
    ...
    <main class="flex-1 transition-all duration-300 overflow-auto" :class="{ 'ml-[-260px]': !isOpen, 'ml-0': isOpen }">
      <Header
        ...
        <template #toggle-sidebar>
          <button v-if="!isOpen" class="p-3 text-white rounded-lg cursor-pointer" @click="toggleSidebar">
        ...
        <template #default>
          <Button
            ...
            icon="fa-star" />
          <Button red label="Sair" @click="logout()" icon="fa-sign-out-alt" />
        </template>
      </Header>
