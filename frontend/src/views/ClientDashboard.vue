  ...
  export default {
    ...
    methods: {
      ...
      viewReservation(reservation) {
        console.log('Visualizar reserva:', reservation.id)
        // this.$router.push({ name: 'reservation-details', params: { id: reservation.id } })
      },
      cancelReservation(reservation) {
        if (confirm(`Deseja realmente cancelar a reserva ${reservation.id} para ${reservation.destino}?`)) {
          ...
          // Chamada API para cancelar reserva
        }
      },
      rateFlight(flight) {
        console.log('Avaliar voo:', flight.id)
        // this.$router.push({ name: 'rate-flight', params: { id: flight.id } })
      },
      logout() {
