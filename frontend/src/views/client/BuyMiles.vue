  ...
  <main class="p-6 max-w-xl mx-auto space-y-6">
    ...
    <form @submit.prevent="confirmPurchase" class="space-y-4">
      <Card>
        <template #default>
          ...
          <Input title="Milhas Recebidas" type="text" icon="fa-star" :placeholder="milhasCalculadas || '0'" disabled />
          <Button type="submit" blue label="Confirmar Compra" :disabled="!form.valor" icon="fa-check" />
        </template>
      </Card>
    </form>
    ...
    <Modal v-if="modalVisible" @close="modalVisible = false">
      <template #content>
        ...
        <p>
          Você comprou <strong>{{ milhasCalculadas }}</strong> milhas por <strong>R$ {{ form.valor }}</strong>
        </p>
        <Button label="Fechar" blue class="mt-4" @click="modalVisible = false" />
      </template>
    </Modal>
  </main>
</template>
...
export default {
  ...
  methods: {
    ...
    confirmPurchase() {
      ...
      if (!valorNumerico || milhas <= 0) {
        this.errorMessage = 'Por Favor, preencha o valor para a compra.'
        return
      }
